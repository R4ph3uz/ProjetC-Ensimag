 ...
 *		where drawing can be done. What is visible on screen is represented by the surface
 *		created by \ref hw_create_window. Other "offscreen" surfaces can be created
 *		by \ref hw_surface_create. Surfaces containing text and images are created by
 ...
 * 		*Must* be called (only once) before any other calls to the hardware interface.
 */
void hw_init(void);

/**
 * @brief	Closes the access to the low-level operating system services and releases
 * 		resources.
 */
void hw_quit(void);
 ...
 *		For example, if *ir is 2, this means that the red channel is stored at
 *		the address of the (4 bytes) pixel plus two.
 ...
 *		alpha channel.
 *
 * @return	A boolean: true means that the surface has an alpha (transparency) channel,
...
/**
 * @brief	Computes the size of a text surface given the font and the text.
 *
 ...
 *				surface.
 */
void hw_text_compute_size		(ei_const_string_t	text,
 ...
 *		WARNING! transparency must be handled when using the returned surface, as explained
 *		in the return parameter below.
 *
 * @param	text		The string of the message.
 * @param	font		The font used to render the text.
 ...
 *				alpha channel of the surface: all pixels have the same color.
 *				Background pixels are transparent, interior pixels are opaque, pixels
 *				at letters' boundary have some transparency.
 */
 ...
 */
ei_surface_t hw_image_load		(ei_const_string_t	filename,
 ...
 * @param	user_param	The user parameter that will be retrievable in the event.
 *
 * @return			The identifier of the scheduled event. Used by \ref hw_event_cancel_app
 * 				to cancel a future execution.
 */
void* hw_event_schedule_app(int ms_delay, ei_user_param_t user_param);

/**
 * @brief	Cancels an application-generated event that was scheduled by \ref hw_event_schedule_app.
 *
 * @param	id		Identifier of the scheduled event, returned by \ref hw_event_schedule_app.
 *
 * @return			True if the scheduled event was found and canceled, false otherwise.
 */
bool hw_event_cancel_app(void* id);
